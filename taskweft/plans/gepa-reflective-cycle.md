...
DSPy-inspired self-optimizing agent primitives for the multiplayer-fabric-taskweft
C++/Elixir NIF layer, ordered by Pareto score.
...
Strict C++ structs or Godot `.tres` Resources defining inputs and outputs. Compile-time
safety and zero runtime parsing overhead replace "vibe-based" prompting with type-safe
interface contracts; required for RECTGTN planner validation.
...
Compressed KV caches hot-swap between agent task-switches. Critical for context folding;
prevents cold-start latency when switching behavioral modes.
...
Basic forward-pass logic runs natively in C++ as asynchronous task nodes interfacing
directly with llama-cpp-turboquant, implementing "Thinking Trajectories" via
...
"Actionable Side Information" generated by failures serializes to storage, preserving
failure context for optimization during background idle cycles.
...
The Genetic-Pareto loop runs as a decoupled background process, evolving instructions
along a Pareto frontier — background evolutionary search maintaining optimal prompt
...
Agents spawn sub-tasks recursively, acting as active readers of their own large memory
buffers and achieving "pseudo-infinite" context.
...
The stateful blackboard serializes to local storage, providing agent continuity across
separate user sessions and world-state reloads.
...
Successful "traces" cache and inject into prompts — a core stabilization primitive for
...
Declarative constraints (e.g., `Assert(target_in_sight)`) trigger backtracking.
Integrated self-correction minimizes hallucination rates during engine tool interaction.
